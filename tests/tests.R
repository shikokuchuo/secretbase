...
test_equal(base64enc(as.raw(c(0xfb, 0xff))), "+/8=")
test_equal(base64enc(""), base64dec(""))
# Base64url tests (url = TRUE):
test_error(base64dec("", url = "x"), "'url' must be a logical value")
test_error(base64dec("+/8", url = TRUE), "input is not valid base64url")
test_error(base64dec("-_8=", url = TRUE), "input is not valid base64url")
test_error(base64dec("c2VjcmV0a", url = TRUE), "input is not valid base64url")
test_equal(base64enc("secret base", url = NA), "c2VjcmV0IGJhc2U")
test_equal(base64enc("s", url = TRUE), "cw")
test_equal(base64enc(as.raw(c(0xfb, 0xff)), url = TRUE), "-_8")
test_equal(base64dec(base64enc("s", url = TRUE), url = TRUE), "s")
test_equal(base64dec(base64enc("secret base", url = TRUE), url = TRUE), "secret base")
test_identical(base64dec(base64enc(as.raw(c(0xfb, 0xff)), url = TRUE), convert = FALSE, url = TRUE), as.raw(c(0xfb, 0xff)))
# Base58 tests:
...
test_identical(cbordec(as.raw(c(0x3b, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00))), -4294967297)  # 64-bit negint -(2^32+1)
